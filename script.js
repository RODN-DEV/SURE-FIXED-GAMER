...
const getStatusIcon = (status) => {
    if (status === 'won') return '<i class="fas fa-check-circle"></i>';
    if (status === 'lost') return '<i class="fas fa-times-circle"></i>';
    return '<i class="fas fa-clock"></i>';
};

const getStatusClass = (status) => {
    if (status === 'won') return 'badge-won';
    if (status === 'lost') return 'badge-lost';
...
// 1. HOME VIEW
const renderHome = () => {
    headerTitle.innerText = "FOOTBALL SIMPLE";
    return `
        <div class="flex flex-col gap-5 mt-4 fade-in">
            ...
            <button onclick="navigate('free-tips')" class="menu-btn w-full py-6 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold text-xl flex items-center justify-between px-6 relative overflow-hidden group">
            ...
            <button onclick="navigate('ultimate')" class="menu-btn w-full py-6 rounded-2xl bg-gradient-to-r from-blue-700 to-indigo-800 text-white font-bold text-xl flex items-center justify-between px-6 border border-blue-400/30">
            ...
            <button onclick="navigate('top-secret')" class="menu-btn w-full py-6 rounded-2xl bg-gradient-to-r from-rose-600 to-pink-700 text-white font-bold text-xl flex items-center justify-between px-6 border border-red-400/30">
                <div class="flex items-center gap-3">
             ...
             <button onclick="navigate('over-under')" class="menu-btn w-full py-6 rounded-2xl bg-gradient-to-r from-violet-600 to-purple-700 text-white font-bold text-xl flex items-center justify-between px-6 border border-purple-400/30">
                <div class="flex items-center gap-3">
            ...
            <button onclick="navigate('btts')" class="menu-btn w-full py-6 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-600 text-white font-bold text-xl flex items-center justify-between px-6 border border-orange-400/30">
                <div class="flex items-center gap-3">
            ...
            <button onclick="navigate('support')" class="menu-btn w-full py-5 rounded-2xl bg-white text-slate-700 font-bold text-lg flex justify-center items-center gap-2 uppercase mt-4">
...
const renderFreeTips = () => {
    headerTitle.innerText = "DAILY FREE TIPS";
...
// 3. OVER/UNDER VIP VIEW
const renderOverUnder = () => {
    headerTitle.innerText = "VIP O/U TIPS";
...
const renderTopSecret = () => {
    headerTitle.innerText = "TOP SPECIAL VIP";
...
// 5. ULTIMATE VIP (Clean White/Blue Style)
const renderUltimate = () => {
    headerTitle.innerText = "ULTIMATE VIP";
    
    let html = `<div class="flex flex-col gap-4 p-4 fade-in pb-20">`;
    
    gamesData.ultimate.forEach(game => {
        const statusClass = getStatusClass(game.status);
        const statusText = game.status === 'won' ? 'WON' : (game.isLive ? 'LIVE' : 'PENDING');
...
// 6. BTTS VIP VIEW
const renderBTTS = () => {
    headerTitle.innerText = "VIP BTTS TIPS";
...
// 7. SUPPORT VIEW
const renderSupport = () => {
    headerTitle.innerText = "SUPPORT / CONTACT";
    return `
                ...
                <div class="space-y-3 text-left">
                    ...
                    <a href="https://t.me/UGAND_J" class="flex items-center p-4 bg-blue-50 rounded-xl border border-blue-100 hover:bg-blue-100 transition group">
                        ...
                        <div>
                            ...
                            <div class="font-bold                    <i class="fas fa-sort-amount-up text-purple-200"></i>
                    <span class="tracking-wide">VIP O/U TIPS</span>
                </div>
                <i class="fas fa-lock text-white/50 text-sm"></i>
            </button>
...
// VIP Views that require password
...
const navigate = (view) => {
    ...
    // Simple active state check for future features
    switch(view) {
